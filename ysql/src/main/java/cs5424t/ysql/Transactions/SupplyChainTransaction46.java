...
import cs5424t.ysql.DAO.Xcnd46.*;
...
import cs5424t.ysql.Entities.Xcnd46.*;
...
@Service
public class SupplyChainTransaction46 implements SupplyChainTransaction{

    @Autowired
    @Qualifier("customerRepositoryXcnd46")
    private CustomerRepositoryXcnd46 customerRepository;

    @Autowired
    @Qualifier("districtRepositoryXcnd46")
    private DistrictRepositoryXcnd46 districtRepository;

    @Autowired
    @Qualifier("itemRepositoryXcnd46")
    private ItemRepositoryXcnd46 itemRepository;

    @Autowired
    @Qualifier("orderLineRepositoryXcnd46")
    private OrderLineRepositoryXcnd46 orderLineRepository;

    @Autowired
    @Qualifier("orderRepositoryXcnd46")
    private OrderRepositoryXcnd46 orderRepository;

    @Autowired
    @Qualifier("stockRepositoryXcnd46")
    private StockRepositoryXcnd46 stockRepository;

    @Autowired
    @Qualifier("warehouseRepositoryXcnd46")
    private WarehouseRepositoryXcnd46 warehouseRepository;

    @PersistenceContext
    @Qualifier("entityManagerXcnd46")
    ...
    public void newOrder(Integer warehouseId, Integer districtId, Integer customerId,
        ...
        DistrictXcnd46 district = districtRepository.findById(new DistrictPK(warehouseId, districtId)).get();
        CustomerXcnd46 customer = customerRepository.findById(new CustomerPK(warehouseId, districtId, customerId)).get();
        WarehouseXcnd46 warehouse = warehouseRepository.findById(warehouseId).get();
        ...
        // create Order object
        OrderXcnd46 order = new OrderXcnd46();
        order.setId(N);
...
//        int j = 1 / 0;
        ...
        for(int i=0;i<itemTotalNum;i++){
            ...
            StockXcnd46 curStock = stockRepository.findByWarehouseIdAndItemId(warehouseId, curItemId);
            ItemXcnd46 curItem = itemRepository.findById(curItemId).get();
            ...
            OrderLineXcnd46 orderLine = new OrderLineXcnd46();
...
//       who placed this order
            OrderXcnd46 oldestOrder = orderRepository.findTopByWarehouseIdAndDistrictIdAndCarrierIdIsNullOrderByIdAsc(warehouseId,districtId);
...
//       (c) Update all the order-lines in X by setting OL DELIVERY D to the current date and time
            ...
            List<OrderLineXcnd46> allOrderLines = orderLineRepository.findAllByWarehouseIdAndDistrictIdAndOrderId(warehouseId,districtId,orderId);
            BigDecimal totalOrderLineAmount = new BigDecimal(0);
            for(OrderLineXcnd46 orderLine : allOrderLines){
...
//        (c) Carrier identifier O CARRIER ID
        OrderXcnd46 lastOrder = orderRepository.findTopByWarehouseIdAndDistrictIdAndCustomerIdOrderByIdDesc(warehouseId, districtId, customerId);
...
//        (e) Data and time of delivery OL DELIVERY D
    ...
    public void stockLevel(Integer warehouseId, Integer districtId, BigDecimal threshold, Integer numLastOrders) {
        ...
        List<OrderLineXcnd46> orderLines;
        ...
        for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
            ...
            for (OrderLineXcnd46 orderLine : orderLines) {
    ...
    public void popularItem(Integer warehouseId, Integer districtId, Integer numLastOrders) {
        ...
        HashMap<Integer, List<OrderLineXcnd46>> orderlineMap = new HashMap<>();
        for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
            OrderXcnd46 order = orderRepository.findById(new OrderPK(warehouseId, districtId, orderId)).get();
            ...
            List<OrderLineXcnd46> mostPopulars = new ArrayList<>();
            BigDecimal maxQuantity = new BigDecimal(-1);
            for (OrderLineXcnd46 ol : orderLines) {
            ...
            for (OrderLineXcnd46 ol : mostPopulars) {
                int itemId = ol.getItemId();
                ItemXcnd46 item = itemRepository.findById(itemId).get();
        ...
        for (int itemId : popularItemIds) {
            ...
            ItemXcnd46 item = itemRepository.findById(itemId).get();
            for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
                List<OrderLineXcnd46> orderLines = orderlineMap.get(orderId);
                for (OrderLineXcnd46 ol : orderLines) {

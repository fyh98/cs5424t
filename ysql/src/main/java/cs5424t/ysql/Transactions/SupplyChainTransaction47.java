...
import cs5424t.ysql.DAO.Xcnd47.*;
...
import cs5424t.ysql.Entities.Xcnd47.*;
...
public class SupplyChainTransaction47 implements SupplyChainTransaction{
    ...
    private CustomerRepositoryXcnd47 customerRepository;

    @Autowired
    @Qualifier("districtRepositoryXcnd47")
    private DistrictRepositoryXcnd47 districtRepository;

    @Autowired
    @Qualifier("itemRepositoryXcnd47")
    private ItemRepositoryXcnd47 itemRepository;

    @Autowired
    @Qualifier("orderLineRepositoryXcnd47")
    private OrderLineRepositoryXcnd47 orderLineRepository;

    @Autowired
    @Qualifier("orderRepositoryXcnd47")
    private OrderRepositoryXcnd47 orderRepository;

    @Autowired
    @Qualifier("stockRepositoryXcnd47")
    private StockRepositoryXcnd47 stockRepository;

    @Autowired
    @Qualifier("warehouseRepositoryXcnd47")
    private WarehouseRepositoryXcnd47 warehouseRepository;
    
    @PersistenceContext
    @Qualifier("entityManagerXcnd47")
    ...
    public void newOrder(Integer warehouseId, Integer districtId, Integer customerId,
        ...
        DistrictXcnd47 district = districtRepository.findById(new DistrictPK(warehouseId, districtId)).get();
        CustomerXcnd47 customer = customerRepository.findById(new CustomerPK(warehouseId, districtId, customerId)).get();
        WarehouseXcnd47 warehouse = warehouseRepository.findById(warehouseId).get();
        ...
        // create Order object
        OrderXcnd47 order = new OrderXcnd47();
        order.setId(N);
...
//        int j = 1 / 0;
        ...
        for(int i=0;i<itemTotalNum;i++){
            ...
            StockXcnd47 curStock = stockRepository.findByWarehouseIdAndItemId(warehouseId, curItemId);
            ItemXcnd47 curItem = itemRepository.findById(curItemId).get();
            ...
            OrderLineXcnd47 orderLine = new OrderLineXcnd47();
...
//       who placed this order
            OrderXcnd47 oldestOrder = orderRepository.findTopByWarehouseIdAndDistrictIdAndCarrierIdIsNullOrderByIdAsc(warehouseId,districtId);
...
//       (c) Update all the order-lines in X by setting OL DELIVERY D to the current date and time
            ...
            List<OrderLineXcnd47> allOrderLines = orderLineRepository.findAllByWarehouseIdAndDistrictIdAndOrderId(warehouseId,districtId,orderId);
            BigDecimal totalOrderLineAmount = new BigDecimal(0);
            for(OrderLineXcnd47 orderLine : allOrderLines){
...
//        (e) Data and time of delivery OL DELIVERY D
    ...
    public void stockLevel(Integer warehouseId, Integer districtId, BigDecimal threshold, Integer numLastOrders) {
        ...
        List<OrderLineXcnd47> orderLines;
        ...
        for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
            ...
            for (OrderLineXcnd47 orderLine : orderLines) {
    ...
    public void popularItem(Integer warehouseId, Integer districtId, Integer numLastOrders) {
        ...
        HashMap<Integer, List<OrderLineXcnd47>> orderlineMap = new HashMap<>();
        for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
            OrderXcnd47 order = orderRepository.findById(new OrderPK(warehouseId, districtId, orderId)).get();
            ...
            List<OrderLineXcnd47> mostPopulars = new ArrayList<>();
            BigDecimal maxQuantity = new BigDecimal(-1);
            for (OrderLineXcnd47 ol : orderLines) {
            ...
            for (OrderLineXcnd47 ol : mostPopulars) {
                int itemId = ol.getItemId();
                ItemXcnd47 item = itemRepository.findById(itemId).get();
        ...
        for (int itemId : popularItemIds) {
            ...
            ItemXcnd47 item = itemRepository.findById(itemId).get();
            for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
                List<OrderLineXcnd47> orderLines = orderlineMap.get(orderId);
                for (OrderLineXcnd47 ol : orderLines) {

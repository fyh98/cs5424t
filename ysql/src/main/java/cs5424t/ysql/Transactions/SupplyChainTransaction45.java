...
import cs5424t.ysql.DAO.Xcnd45.*;
...
@Service
public class SupplyChainTransaction45 implements SupplyChainTransaction{

    @Autowired
    @Qualifier("customerRepositoryXcnd45")
    private CustomerRepositoryXcnd45 customerRepository;

    @Autowired
    @Qualifier("districtRepositoryXcnd45")
    private DistrictRepositoryXcnd45 districtRepository;

    @Autowired
    @Qualifier("itemRepositoryXcnd45")
    private ItemRepositoryXcnd45 itemRepository;

    @Autowired
    @Qualifier("orderLineRepositoryXcnd45")
    private OrderLineRepositoryXcnd45 orderLineRepository;

    @Autowired
    @Qualifier("orderRepositoryXcnd45")
    private OrderRepositoryXcnd45 orderRepository;

    @Autowired
    @Qualifier("stockRepositoryXcnd45")
    private StockRepositoryXcnd45 stockRepository;

    @Autowired
    @Qualifier("warehouseRepositoryXcnd45")
    private WarehouseRepositoryXcnd45 warehouseRepository;
    ...
    public void newOrder(Integer warehouseId, Integer districtId, Integer customerId,
        ...
        WarehouseXcnd45 warehouse = warehouseRepository.findById(warehouseId).get();
        ...
        // create Order object
        OrderXcnd45 order = new OrderXcnd45();
        order.setId(N);
...
//        int j = 1 / 0;
        ...
        for(int i=0;i<itemTotalNum;i++){
            ...
            StockXcnd45 curStock = stockRepository.findByWarehouseIdAndItemId(warehouseId, curItemId);
            ItemXcnd45 curItem = itemRepository.findById(curItemId).get();
            ...
            OrderLineXcnd45 orderLine = new OrderLineXcnd45();
...
//        (e) Data and time of delivery OL DELIVERY D
    ...
    public void stockLevel(Integer warehouseId, Integer districtId, BigDecimal threshold, Integer numLastOrders) {
        ...
        List<OrderLineXcnd45> orderLines;
        ...
        for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
            ...
            for (OrderLineXcnd45 orderLine : orderLines) {
    ...
    public void popularItem(Integer warehouseId, Integer districtId, Integer numLastOrders) {
        ...
        HashMap<Integer, List<OrderLineXcnd45>> orderlineMap = new HashMap<>();
        for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
            OrderXcnd45 order = orderRepository.findById(new OrderPK(warehouseId, districtId, orderId)).get();
            ...
            List<OrderLineXcnd45> mostPopulars = new ArrayList<>();
            BigDecimal maxQuantity = new BigDecimal(-1);
            for (OrderLineXcnd45 ol : orderLines) {
            ...
            for (OrderLineXcnd45 ol : mostPopulars) {
                int itemId = ol.getItemId();
                ItemXcnd45 item = itemRepository.findById(itemId).get();
        ...
        for (int itemId : popularItemIds) {
            ...
            ItemXcnd45 item = itemRepository.findById(itemId).get();
            for (int orderId = Math.max(N-numLastOrders, 0); orderId < N; orderId++) {
                List<OrderLineXcnd45> orderLines = orderlineMap.get(orderId);
                for (OrderLineXcnd45 ol : orderLines) {
